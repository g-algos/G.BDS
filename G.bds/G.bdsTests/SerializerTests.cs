...
namespace G.bds.Tests
{
    ...
    {
        private static BDS _bds = new BDS(
        ...
        {
            Serializer.Save(_bds, Path.GetDirectoryName(Assembly.GetExecutingAssembly().Location)!,"test");
            Assert.IsTrue(File.Exists(Path.Combine(Path.GetDirectoryName(Assembly.GetExecutingAssembly().Location)!, "test"));
        }
